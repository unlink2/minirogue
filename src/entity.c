...
int mrg_entity_spawn(struct mrg_state *state, struct mrg_entity *entity) {
  ...
  mrg_entity_init_type(e, entity->type);
  e->type = entity->type;
  e->flags = entity->flags | e->flags;
  e->x = entity->x;
  e->y = entity->y;
  e->tileset_id = entity->tileset_id;
